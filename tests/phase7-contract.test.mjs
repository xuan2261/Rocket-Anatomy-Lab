...
test('CI has a pinned-environment visual regression lane', () => {
  ...
  assert.match(ci, /name: Visual regression[\s\S]*site_artifact\.py restore[\s\S]*npm run test:visual[\s\S]*site_artifact\.py proof visual/)
  assert.match(ci, /name: Visual regression[\s\S]*needs: verify/)
